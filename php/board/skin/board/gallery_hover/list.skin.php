...
add_javascript('<script type="text/javascript" src="'.$board_skin_url.'/highslide/highslide.js"></script>', 0); 


$kb_zoom = '1'; //1일 경우 zoom기능 사용.
    ...
    <ul id="gall_ul">
        ...
        <li class="gall_li <?php if ($wr_id == $list[$i]['wr_id']) { ?>gall_now<?php } ?>" style="<?php echo $style ?>width:<?php echo $board['bo_gallery_width'] ?>px">
            ...
            <ul class="gall_con">
                <li class="gall_href">
                    <?php 
                    $kb_wr_id = $list[$i][wr_id];
                    $sql_img = sql_fetch ("select* from g5_board_file where wr_id = '$kb_wr_id' and bo_table='$bo_table' and bf_no = '0'");
					$zoom_img_url = '/data/file/'.$bo_table.'/'.$sql_img[bf_file];
					...
					if($kb_zoom == '1' and !$list[$i]['is_notice'] and $sql_img['bf_file']){
                    	echo '<a href="'.$zoom_img_url.'"  style="'.$zoomimg_style.'" class="highslide" onclick="return hs.expand(this)">';
						echo '<img src="'.$board_skin_url.'/img/btn_photo.png">';
						echo '</a>';
                     }?>
                    ...
                    <?php } else {
                        ...
                        if($thumb['src']) {
                            $img_content = '<img src="'.$thumb['src'].'" alt="'.$thumb['alt'].'" width="'.($board['bo_gallery_width']-1).'" height="'.$board['bo_gallery_height'].'">';
                        } else {
        ...
        <div class='highslide-caption' id='the-caption'>
            ...
            <nobr><a href="#" class="highslide-move control" onclick="return false">위치이동</a></nobr>
